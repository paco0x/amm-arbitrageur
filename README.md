...
- 这里的 Token 称为 Quote Token，它可以是任意代币，即使毫无价值也没问题，因为套利结束后不会保留 Quote Token
...
- 假设有交易对 Pair0 和 Pair1，他们直接有差价，即满足套利条件
- 调用合约开始套利
- 合约计算 Quote Token 的价格，假设 Pair0 中 Quote Token 价格较低，那么
- 通过 flash swap ，从 Pair0 中借取数量为 x 的 Quote Token，此时我们产生了一笔负债，在交易结束前，我们需要向 Pair0 偿还数量为 y1 的 Base Token
- 将借来的 Quote Token 在 Pair1 中全部卖出，得到数量为 y2 的 Base Token
- 偿还 Pair0 Base Token，数量为 y1
- 交易结束，净利润为 y2 - y1
...
我们需要求出当利润最大时，x 的值，此时即为我们需要借出的 Quote Token 数量。先对上面的函数求导：
...
合约提供了 `getProfit(address pool1, address pool2)` 接口，可以计算出两个交易对之间，当前套利的最大利润（以 Base Tokne计价）。
